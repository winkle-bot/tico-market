...
test.describe('Stabilization flows', () => {
  ...
  test('offline saved searches replay after reconnect without leaving queued state behind', async ({
    page,
  }) => {
    await seedAuthenticatedUser(page);
    ...
    const savedSearchFingerprint =
      '{"query":"papaya","categories":[],"listingKind":null,"minPrice":null,"maxPrice":null,"sort":"newest"}';
    let savedSearches: Array<Record<string, unknown>> = [];
    let replayPostCount = 0;
    let replayPostBody: Record<string, unknown> | null = null;
    ...
    await page.route('**/api/saved-searches', async (route) => {
      ...
      replayPostCount += 1;
      replayPostBody = JSON.parse(route.request().postData() || '{}') as Record<string, unknown>;
      savedSearches = [
        {
          id: 'saved-search-replayed',
          name: 'papaya',
          query: 'papaya',
          categories: [],
          sort: 'newest',
    ...
    await page.getByLabel('Search listings').fill('papaya');
    await page.getByRole('button', { name: 'Save Alert' }).click();

    expect(replayPostCount).toBe(0);
    await expect(page.getByText('Search alert queued for sync')).toBeVisible();
    ...
    await expect(page.getByLabel('Remove saved search papaya')).toBeVisible();
    await expect
      .poll(async () =>
    ...
    await expect.poll(() => replayPostCount).toBe(1);
    expect(replayPostBody).toEqual({
      query: 'papaya',
      categories: [],
      sort: 'newest',
      alertEnabled: true,
    ...
    await expect
      ...
      .toBe(0);
    await expect(page.getByLabel('Remove saved search papaya')).toBeVisible();
    await expect(page.getByText('Offline mode active')).toHaveCount(0);
  });

  test('offline feria follow toggles collapse to the final queued intent on reconnect', async ({
    page,
  }) => {
    await seedAuthenticatedUser(page);
    await seedOnlineController(page);

    const followMethods: string[] = [];

    await page.route('**/api/ferias/organic-saturday/follow', async (route) => {
      followMethods.push(route.request().method());
      return json(route, {
        feriaId: 'feria-1',
        isFollowing: false,
        followerCount: 12,
    ...
    await page.getByRole('button', { name: 'Follow' }).click();
    await expect(page.getByText('Feria follow queued')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Following' })).toBeVisible();
    await expect(followersRow.getByText('13', { exact: true })).toBeVisible();

    await page.getByRole('button', { name: 'Following' }).click();
    await expect(page.getByText('Feria unfollow queued')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Follow' })).toBeVisible();
    await expect(followersRow.getByText('12', { exact: true })).toBeVisible();
    ...
    await expect
      .poll(async () =>
        page.evaluate(
          () =>
            JSON.parse(window.localStorage.getItem('tico_offline_mutation_queue') || '[]').map(
    ...
    await expect.poll(() => followMethods).toEqual(['DELETE']);
    await expect
      .poll(async () =>
        page.evaluate(() => JSON.parse(window.localStorage.getItem('tico_offline_mutation_queue') || '[]').length)
      )
